...
def iter_fcurves_from_animdata(animdata):
    ...
    # Prefer layered / slotted actions (Blender 4.4+)
...
def iter_fcurves_from_animdata(animdata):
    ...
    Supports:
      ...
      - slotted actions: action.layers[*].strips[*].channelbags[*].fcurves filtered by animdata.action_slot
    """
    if not animdata:
        return
    action = getattr(animdata, "action", None)
    ...
    slot = getattr(animdata, "action_slot", None)
    if slot is None:
        # If slot isn't assigned, try first slot as a reasonable fallback.
        slots = getattr(action, "slots", None)
    ...
    for layer in action.layers:
        ...
        for strip in layer.strips:
            ...
            for cb in strip.channelbags:
                cb_slot = getattr(cb, "slot", None)
                if slot is not None and cb_slot is not None and cb_slot != slot:
                    continue
...
def _initial_refresh():
    try:
        if bpy.context and bpy.context.scene and hasattr(bpy.context.scene, "anim_offset"):
            refresh_channels(bpy.context)
    except Exception:
        pass
